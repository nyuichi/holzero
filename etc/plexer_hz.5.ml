...
(* - OCaml lexer adjusted with support backquotes in the lexer                *)
(*                                                                            *)
(* by Mark Adams                                                              *)
...
(* This file is created by taking the Camlp5 source code file 'lib/plexer.ml' *)
(* (Camlp5 version 5.12) and adjusting a few lines.  The changed lines end    *)
(* with the ML comment "HZ".                                                  *)


(* camlp5r pa_lexer.cmo *)
(* $Id: plexer.ml 1911 2008-08-02 02:32:13Z deraugla $ *)
(* Copyright (c) INRIA 2007-2008 *)
...
value rec holquote ctx bp =                                             (* HZ *)
  lexer                                                                 (* HZ *)
  [ "`"/                                                                (* HZ *)
  | (any ctx) (holquote ctx bp)!                                        (* HZ *)
  | -> err ctx (bp, $pos) "quotation not terminated" ]                  (* HZ *)
;                                                                       (* HZ *)
...
value next_token_after_spaces ctx bp =
  ...
  | "`"/ (holquote ctx bp)! -> ("QUOTATION", "hol:" ^ $buf)             (* HZ *)
  | "$"/ (dollar ctx bp)!
  | [ '!' | '=' | '@' | '^' | '&' | '+' | '-' | '*' | '/' | '%' ] ident2! ->
